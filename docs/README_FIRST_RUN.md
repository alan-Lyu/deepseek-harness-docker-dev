...
Install the AppArmor profile, create the workspace, then build and start:

安装 AppArmor 配置、创建工作区，然后构建并启动：
...
mkdir -p workspace
SSH_PUB_KEY="$(cat ~/.ssh/id_ed25519.pub)" docker compose up -d --build
```

After initialization, ordinary starts need no key variable:

初始化完成后，普通启动无需再次提供公钥变量：
...
Replace the example `DSH_IMAGE` value with the registry image name. The AppArmor profile must already be installed from this repository.

将示例中的 `DSH_IMAGE` 值替换为镜像仓库中的实际名称；宿主机必须已经从本仓库安装 AppArmor 配置。
